...
Memory 文件写入后，还需要在 `MEMORY.md` 索引中注册，否则下次会话不会自动加载。

在 MEMORY.md 中追加一行（用 Edit 工具在文件末尾添加）：

```
- [飞书日记配置](feishu-diary-config.md) — Base/文档 token、标题格式、字段映射
```

> 这样用户在日常对话中说「我日记里提到的那个面试怎么样了」，AI 就能从 Memory 中读取 Base 位置并查询日记内容。
